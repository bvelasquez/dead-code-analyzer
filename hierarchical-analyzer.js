...
const outputFile = process.argv[3] || "analysis-results.json";
...
if (targetDir !== process.cwd()) {
  try {
    ...
    console.error(`Error changing to directory ${targetDir}:`, error.message);
    process.exit(1);
  }
}
...
function findSourceFiles() {
  try {
    ...
    if (excludedFiles.length > 0) {
      console.log(
        `${colors.cyan}📝 Excluded ${excludedFiles.length} test/story files from analysis${colors.reset}`,
      );
    }
  ...
  try {
    ...
    importPatterns.forEach((pattern) => {
      ...
      while ((match = pattern.exec(content)) !== null) {
        ...
        // Only process relative imports and src/ imports
...
function findEntryPoints() {
  ...
  const patterns = ["index.*", "App.*", "main.*"];

  patterns.forEach((pattern) => {
    try {
      const output = execSync(`find src/ -name "${pattern}" -type f`, {
        encoding: "utf8",
      });
      const files = output
        .trim()
      ...
      files.forEach((file) => {
        entryPoints.push(
          file.replace(/^src\//, "").replace(/\.(js|jsx|ts|tsx)$/, ""),
        );
      });
    } catch (error) {
  ...
  if (unreachableFiles.length > 0) {
    ...
    unreachableFiles.slice(0, 5).forEach((file) => {
      ...
      if (analysis.chains.length > 0 && !analysis.isOrphaned) {
        ...
        console.log(
          `      Root: ${sampleChain.rootFile} (${sampleChain.isRootReachable ? "reachable" : "unreachable"})`,
        );
      }
      console.log("");
    });
